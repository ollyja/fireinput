...
var FireinputEncoding = 
{
    ...
    getEncodedString: function(origStr, encoding)
    {
       ...
       {
          if(encoding == ENCODING_ZH)
             encodedString += this.encoder.switchToZH(str[i]); 
          else 
             encodedString += this.encoder.switchToBig5(str[i]); 
       }
    ...
    switchEncodingAsync: function(str, encoding, options)
    {
      if(!this.encoder)
       ...
       for(var i=0; this.currentStep<str.length && i<this.maxStep; this.currentStep++)
       {
          var charCode = str.charAt(this.currentStep);
          if(charCode >= '\u4e00')
          {
             if(this.encodingName == ENCODING_ZH)
                this.encodedString += this.encoder.switchToZH(charCode);
             else 
                this.encodedString += this.encoder.switchToBig5(charCode);

             // control how many chars have been translated 
             i++; 
          }
          else 
          { 
             this.encodedString += charCode; 
          }
       }

       if(this.currentStep < str.length)
       {
          this.origString = str;
          var self = this; 
       ...
       else if(this.options)
       {
          if(this.options.oncomplete)
             this.options.oncomplete(this.encodedString);
       }
    },
