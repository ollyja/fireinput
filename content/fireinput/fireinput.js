...
var Fireinput = 
{
       ...
       switch(this.myIMESchema)
       {
          ...
          default:
             ...
             if(!ime.isEnabled() && typeof(schema) == 'undefined')
                 return this.getDefaultIME(WUBI_86); 
          break; 
       }
       ...
       {  
          ...
          if(value != null)
          {
             for(var index = 0; ; index++)
             {
                ...
                {
                   if(element.selectedItem.style.display == "none")   
                      continue; 
    ...
    toggleInputMethod: function()
    {
       ...
       else if((this.myIMESchema == WUBI_86 || this.myIMESchema == WUBI_98) && 
          method != this.myIMESchema)
       {
          this.myIME = null; 
	...
	if(charCode > KeyEvent.DOM_VK_0 && charCode <= KeyEvent.DOM_VK_9)
	{
          if(this.myIMEInputBarStatus)
          {
             FireinputLog.debug(this, "this.myComposeEnabled: " + this.myComposeEnabled + ", press number key: " + key);
             if(this.myComposeEnabled || FireinputComposer.hasSet())
             {
	  ...
	  // open IME input window 
	     ...
	     {
                var id = document.getElementById("fireinputIMEContainer"); 
                id.showPopup(target, -1, -1, "popup", "bottomleft", "topleft"); 
	     }
    ...
    insertCharToComposer: function (event, i, cas)
    {
       ...
       if(result)
       {
          ...
          if(!composeWasEnabled)
          {
             ...
             var idf = document.getElementById("fireinputField");
             idf.value = ""; 
             // from event. Should not keep the global value 
             if(event)
             {
                this.myInputChar = ""; 
             }
    ...
    insertCharToTarget: function (event, target, i, hideInput)
    {
       if(this.myIMEInputBarStatus)
       {
          ...
          this.insertAllCharsToTarget(target, hideInput, {key: key, word: word, value: value}); 
       }
    }, 
    ...
    IMEWindowHiding: function()
    {
       ...
       FireinputLog.debug(this, "this.myComposeEnabled: " + this.myComposeEnabled);
       if(this.myIMEInputFieldFocusedStatus || this.myComposeEnabled)
       {
          FireinputUtils.setFocus(this.myTarget.target);
       }
